...
def main():
    print("🚀 AUTOMATIZACIÓN CARDTRADER/CARDMARKET - MÚLTIPLES CARTAS")
    print("=" * 60)
    
    # Seleccionar plataforma
    print("🌐 Selecciona la plataforma:")
    print("1. CardTrader")
    print("2. CardMarket")
    opcion = input("Ingresa tu opción (1 o 2): ").strip()
    
    plataforma = "cardtrader" if opcion == "1" else "cardmarket"
    
    # Leer lista de cartas desde el usuario
    cartas_input = input("📝 Ingresa las cartas (una por línea): ").strip()
    if not cartas_input:
        return
    
    # Dividir la lista original por líneas
    lista_cartas_original = [carta.strip() for carta in cartas_input.split('\n') if carta.strip()]
    
    if not lista_cartas_original:
        print("❌ No se ingresaron cartas válidas")
        return
    
    # Crear el automator múltiple
    multi_automator = MultiCardAutomator(plataforma)
    
    try:
        print(f"\n🌐 Plataforma seleccionada: {plataforma.upper()}")
        print(f"✅ Filtros aplicados: Vendedores UE (sin UK) + Cartas Inglés + SOLO NM + Zero")
        print("💡 Se cerrará cada pestaña después de procesar cada carta")
        print("🔄 Sistema de reintentos: Si no hay vendedores, probará con la siguiente carta más barata")
        print("🧹 Sistema de procesamiento:")
        print("   - '2 Mountain' → 2 copias de Mountain")
        print("   - 'Sauron, The Dark Lord' → 'Sauron The Dark Lord' (una sola carta)")
        print("   - Se respetan las cantidades y nombres con comas")
        print("📦 Sistema de cantidades: Se buscarán vendedores con stock suficiente")
        
        # Mostrar transformación
        lista_procesada = multi_automator.limpiar_y_procesar_lista_cartas(lista_cartas_original)
        contador = Counter(lista_procesada)
        print(f"\n📋 Lista original: {len(lista_cartas_original)} líneas")
        print(f"📋 Lista procesada: {len(contador)} cartas únicas, {len(lista_procesada)} copias totales")
        
        # Procesar todas las cartas
        exitos, fallos = multi_automator.procesar_lista_cartas(lista_cartas_original)
        
        # Mostrar resumen
        multi_automator.mostrar_resumen()
