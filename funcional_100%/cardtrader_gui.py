...
from ListCart import MultiCardAutomator

class CardTraderGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("CardTrader Automator")
    ...
    def setup_ui(self):
        ...
        cleanup_info = ttk.Label(main_frame, 
                                ...
                                foreground="blue",
                                wraplength=700)
        cleanup_info.grid(row=2, column=0, columnspan=2, sticky=tk.W, pady=(0, 10))
        ...
        self.cards_text = scrolledtext.ScrolledText(main_frame, 
                                                   ...
                                                   width=60,
                                                   font=("Consolas", 10))
        self.cards_text.grid(row=3, column=0, columnspan=2, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))
        ...
        status_label.grid(row=8, column=0, columnspan=2, sticky=tk.W, pady=(5, 0))
...
3 Cultivate"""
    ...
    def start_automation(self):
        ...
        # Actualizar estado
        self.update_status(f"Procesando {len(cards)} cartas...")
        
        # Ejecutar en un hilo separado para no bloquear la GUI
        thread = threading.Thread(target=self.run_automation, args=(cards,))
        thread.daemon = True
        thread.start()
    
    def run_automation(self, cards):
        """Ejecuta la automatización (en hilo separado)"""
        try:
            self.automator = MultiCardAutomator()
            ...
            # Procesar cartas
            self.log_message("=" * 60)
            self.log_message("🚀 INICIANDO AUTOMATIZACIÓN CARDTRADER")
            self.log_message("=" * 60)
            ...
            # Mostrar resultados
            self.root.after(0, self.automation_completed, exitos, fallos, cards)
            
        except Exception as e:
            self.root.after(0, self.automation_failed, str(e))
    
    def automation_completed(self, exitos, fallos, cards):
        ...
        self.update_status(f"Completado: {exitos}/{len(cards)} cartas añadidas")
