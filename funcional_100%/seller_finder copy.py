...
class SellerFinder:
    ...
    def buscar_vendedores_con_zero_real(self, condiciones, cantidad_necesaria=1):
        ...
        try:
            ...
            # ORDENAR por stock disponible (más cercano a la cantidad necesaria) y luego por precio
            vendedores_filtrados.sort(key=lambda x: (
                -x.get('stock_suficiente', 0),  # Primero los que tienen stock suficiente
                abs(x.get('stock_disponible', 0) - cantidad_necesaria),  # Luego los más cercanos a la cantidad
                x['precio']  # Finalmente por precio
            ))
